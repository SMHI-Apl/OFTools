...
defineTypeNameAndDebug(kEpsilon_canopy, 0);
addToRunTimeSelectionTable(RASModel, kEpsilon_canopy, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

kEpsilon_canopy::kEpsilon_canopy
(
...
tmp<volSymmTensorField> kEpsilon_canopy::R() const
{
...
tmp<volSymmTensorField> kEpsilon_canopy::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> kEpsilon_canopy::divDevReff(volVectorField& U) const
{
    return
    (
...
void kEpsilon_canopy::correct()
{
    ...
    // Added source term CpEps1*Fk*epsilon/k
