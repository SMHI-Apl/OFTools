...
hargreavesABL::hargreavesABL(const vectorField& p, const dictionary& dict)
:
...
hargreavesABL::hargreavesABL(const hargreavesABL& blpvf)
:
...
void hargreavesABL::autoMap(const fvPatchFieldMapper& m)
{
    z0_.autoMap(m);
    zGround_.autoMap(m);
    Ustar_.autoMap(m);
...
void hargreavesABL::rmap
(
    const hargreavesABL& blptf,
...
tmp<vectorField> hargreavesABL::U(const vectorField& p) const
{
    scalarField Un
    (
        (Ustar_/kappa_)
...
tmp<scalarField> hargreavesABL::k(const vectorField& p) const
{
    return sqr(Ustar_)/sqrt(Cmu_);
}


tmp<scalarField> hargreavesABL::epsilon(const vectorField& p) const
{
    return pow3(Ustar_)/(kappa_*((zDir_ & p) - zGround_ + z0_));
}

tmp<scalarField> hargreavesABL::omega(const vectorField& p) const
{
  return Ustar_/(sqrt(beta_)*kappa_*((zDir_ & p) - zGround_ + z0_));
}




void hargreavesABL::write(Ostream& os) const
{
