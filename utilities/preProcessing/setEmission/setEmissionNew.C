  ...
  for(label i = 0; i < emissionGroups.size(); i++)
    {
      ...
      DynamicList<label> roadPatchList;
      DynamicList<word> roadPatchNameList;
      DynamicList<scalar> roadPatchWeightList;
      ...
      for(label j=0;j<roads.size();j++)
	{
	  ...
	  if(roadEmissionGroup==i and roadWeight>0.0 )
	    {
	      roadPatchList.append(roadIndex);
	      roadPatchNameList.append(roadName);
	      roadPatchWeightList.append(roadWeight);
	    }
	}
      ...
      Info << endl <<"Emission group summary for: " << emissionGroupNames[i] << endl
	   << "Road with non zero weights: "<<endl;
      scalar weightSum=0;
      for(label j=0;j<roadNameList.size();j++)
	{
	  Info << tab << j+2 << ": " << roadNameList[j]<< " weight: " << roadPatchWeightList[j] << endl;
	  weightSum=weightSum+roadPatchWeightList[j];
	}
